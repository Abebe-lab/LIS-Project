...
//prettier-ignore
import { SatelliteAltRounded as SatelliteIcon, DirectionsOff as DirectionsOffIcon, FilterListOff as FilterListOffIcon, 
  FilterList as FilterListIcon, ArrowRight as ArrowRightIcon, Cancel, } from "@mui/icons-material";
...
const ViewUnapprovedSplitAndMerge = () => {
  ...
  const handleApproveImport = async () => {
    ...
    try {
      const result = await UpdateAndGetResponse("splitAndMerge/approve");
      if (result) {
  ...
  const handleDeclineImports = async () => {
    ...
    try {
      ...
      const result = await UpdateAndGetResponse("splitAndMerge/decline");
      console.log(result);
      if (result) {
        toast.success(result.message, { position: "top-right", autoClose: 3000 });
      }
    ...
    <>
      ...
      <Grid container spacing={2}>
        ...
        {decodedUser &&
          ...
          decodedUser?.role?.toUpperCase() === "HEAD" && (
...
export default ViewUnapprovedSplitAndMerge;
